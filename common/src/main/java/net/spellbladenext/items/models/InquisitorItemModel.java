...
public class InquisitorItemModel extends AnimatedGeoModel<InquisitorSet> {
    ...
    public Identifier getModelResource(InquisitorSet orb) {
        ...
        if(orb.slot == EquipmentSlot.CHEST){
            return new Identifier(SpellbladeNext.MOD_ID,"geo/inquisitor_chest.json");

        }
        if(orb.slot == EquipmentSlot.FEET){
            return new Identifier(SpellbladeNext.MOD_ID,"geo/inquisitor_feet.json");

        }
        if(orb.slot == EquipmentSlot.LEGS){
            return new Identifier(SpellbladeNext.MOD_ID,"geo/inquisitor_legs.json");
